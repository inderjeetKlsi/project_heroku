...
$(document).ready(function(){
    $('.anova-login-password').keyup(function(){
        var value = $(this).val();
        var id = $(this).attr('id');
        var valueLength = value.length;
        if(parseInt(valueLength) < 8)
        {
            $('#' + id).next().remove();
            $('#' + id).parent().append('<span class="help-block pull-left field-info"> Password must be at least 8 characters.</span>');
        }
        else
        {
            $('#' + id).next().remove();
...
function in_line_validation()
{
	...
	$('.unique-email').blur(function(){
		validate_unique_email_field(this, true)
	});
	
	$('.registered-email').blur(function(){
		validate_registered_email_field(this, true,'0')
	});
	$('.already-registered-email').blur(function(){
		validate_registered_email_field(this, true,'1')
	});
	...
	$('.registered-user-email').blur(function(){
		validate_email_exist(this, true)
	});
	
   	$('.handicap').blur(function(){
		validate_handicap_field(this, true)
	});
...
{	
	...
	check_add_parent(id_input, 1, 'has-error');
        ...
        if(isInputGroup)
        {
            ...
            $('#' + id_input).parent().parent().append('<span class="help-block pull-left"><i class="fa fa-times-circle"></i> ' + text_notify.replace('e.g. Olympic Club', 'Club Name') + '</span>');	
        }
        else
        {
            $('#' + id_input).next().remove();
        
            $('#' + id_input).parent().append('<span class="help-block pull-left"><i class="fa fa-times-circle"></i> ' + text_notify.replace('e.g. Olympic Club', 'Club Name') + '</span>');	
        }
...
function validate_unique_email_field(obj, is_auto_save)
{
	email = replaceAll($.trim($(obj).val()), ' ', '');
	email = replaceAll(email,',', '.');	
	email = removeAllTrailingDotsAndCommasInEmail(email);
...
function validate_registered_email_field(obj, is_auto_save,idUserFlag)
{
	email = replaceAll($.trim($(obj).val()), ' ', '');
	email = replaceAll(email,',', '.');	
	email = removeAllTrailingDotsAndCommasInEmail(email);
	...
	{		
		...
		if(validateEmail(email))
		{
			$.ajax({
		    	type: "POST",
		      	url: __BASE_URL__ + "/users/checkCustomerAlreadyExist",
		  		async: false,
		      	data: "id="+id+"&szEmail="+email+"&idUser=0&idUserFlag="+idUserFlag,
		      	success: function(result){								
					if(result != '')
					{
						ar_result = result.split('||||');
						if(ar_result[0] == "SUCCESS")
						{
							display_success_notification('szEmail', ar_result[1]);
							if(AUTO_SAVE && is_auto_save) autoSubmit(obj);
						}
						else
						{
							display_error_notification('szEmail', ar_result[1]);
						}
					}
				}
			});
...
function validate_email_exist(obj, is_auto_save)
{
	email = replaceAll($.trim($(obj).val()), ' ', '');
	email = replaceAll(email,',', '.');	
	email = removeAllTrailingDotsAndCommasInEmail(email);
		...
		if(validateEmail(email))
		{
			$.ajax({
		    	type: "POST",
		      	url: __BASE_URL__ + "/users/checkCustomerAlreadyExist",
		  		async: false,
		      	data: "id="+id+"&szEmail="+email+"&idUser=0&loginFlag=1",
		      	success: function(result){								
					if(result != '')
					{
						ar_result = result.split('||||');
...
function validate_handicap_field(obj, is_auto_save)
{
	var require_value = $.trim($(obj).val());
	$(obj).val(require_value);
	var id = $(obj).attr('id');
	...
	else
	{
        ...
        checkPlayerCategorySelected();
}

function checkPlayerCategorySelected()
{
    var idPlayerCategory = $("#idPlayerCategory").val();
    
    if($.trim(idPlayerCategory) == '')
    {
        notify = "Please select player category.";
        display_error_notification('idPlayerCategoryError', notify);
    }
    else
    {
        display_success_notification('idPlayerCategoryError', '');
    }
}
